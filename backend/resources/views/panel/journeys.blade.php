...
<div class="journey-workbench">
    <section class="visit-list" aria-label="{{ __('panel.visits') }}">
        ...
        <a @class(['visit-row', 'is-selected' => $selectedId == $journey->id]) aria-current="{{ $selectedId == $journey->id ? 'true' : 'false' }}" href="{{ route('panel.sites.reports', $site).'?'.http_build_query(array_merge(request()->except(['stepsPage']), ['journeyId' => $journey->id])).'#journey-detail' }}">
            <div class="visit-top"><span class="browser-token"><x-panel.icon name="finger-print" /><bdi title="{{ $journey->anonymous_id }}">{{ substr(hash('sha256', $journey->anonymous_id), 0, 8) }}</bdi></span><span class="visit-time" dir="ltr">{{ \Illuminate\Support\Carbon::parse($journey->started_at)->format('m/d · H:i') }}</span></div>
    ...
    @if ($first)
        ...
        <div class="detail-facts"><span><x-panel.icon name="calendar-days" /><time dir="ltr">{{ $first->occurred_at->format('Y/m/d · H:i') }} UTC</time></span><span><x-panel.icon name="queue-list" />{{ $steps->total() }} {{ __('panel.step_count') }}</span><span><x-panel.icon name="clock" />{{ __('panel.recorded_span') }}: {{ $span >= 60 ? floor($span / 60).' '.__('panel.minutes').' '.($span % 60).' '.__('panel.seconds') : $span.' '.__('panel.seconds') }}</span></div>
